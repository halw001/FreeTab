...
function EditTabDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{tr('editTab')}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 pt-2">
          <div className="space-y-2">
            <label className="text-sm font-medium" style={{ color: themeColors.textSecondary }}>{tr('title')}</label>
            <Input
              value={title}
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium" style={{ color: themeColors.textSecondary }}>{tr('url')}</label>
            <Input
              value={url}
      ...
      <div className="group/header flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover/header:opacity-100 transition-opacity">
          <button
            ...
            style={{ color: themeColors.textMuted }}
            title={group.pinned ? tr('unpin') : tr('pin')}
          >
            {group.pinned ? (
          ...
          {group.tabs.length === 0 ? (
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>{tr('deleteGroupConfirmTitle')}</AlertDialogTitle>
                  <AlertDialogDescription>
                    {tr('deleteGroupConfirmDesc')}
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
        ...
        >
          {group.tabs.length === 0 ? (
            <div
              ...
              style={{ borderColor: themeColors.emptyBorder, color: themeColors.emptyText }}
            >
              {tr('dragTabsHere')}
            </div>
          ) : (
...
export default function MainContent() {
  ...
  return (
    <main className="flex-1 flex flex-col min-w-0" style={{ backgroundColor: themeColors.mainContentBg }}>
      {currentView === 'home' && (
        <header
        ...
        {isEditingName ? (
          ...
          <h1
            ...
            title={tr('clickToEditSpaceName')}
          >
            {currentSpace?.name ?? tr('unknown')}
          </h1>
        )}
        <div className="flex items-center gap-2">
          ...
          >
            <Search size={16} />
            <span className="hidden sm:inline">{tr('searchTabs')}</span>
          </button>
          <button
          ...
          <>
            {sortedGroups.length === 0 ? (
              <div className="h-full flex items-center justify-center">
                <p className="text-lg" style={{ color: themeColors.emptyText }}>
                  {tr('emptyState')}
                </p>
              </div>
            ) : (
